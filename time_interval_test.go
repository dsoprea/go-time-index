...
func TestTimeIntervalAddDuplicate(t *testing.T) {
	...
	ti1 := TimeInterval{From: left1, To: right1}

	tis := make(TimeIntervalSlice, 0)

	tis = tis.Add(left1, right1, nil)
	tis = tis.Add(left1, right1, nil)

	if len(tis) != 1 {
		t.Fatalf("Duplicate added.")
	} else if tis[0].From != ti1.From || tis[0].To != ti1.To {
...
func checkTestAddOrder(description string, t *testing.T, unsortedIntervals []TimeInterval, sortedIntervals []TimeInterval) {
	...
	for i, ti := range sortedIntervals {
		if tis[i].From != ti.From || tis[i].To != ti.To {
...
func TestTimeIntervalAddDifferentStartTimes(t *testing.T) {
	...
	ti1 := TimeInterval{From: left1, To: right1}
	ti2 := TimeInterval{From: left2, To: right2}
	ti3 := TimeInterval{From: left3, To: right3}
...
func TestTimeIntervalAddSameStartTime(t *testing.T) {
	...
	*/
	...
	ti1 := TimeInterval{From: left1, To: right1}
	ti2 := TimeInterval{From: left1, To: right2}
	ti3 := TimeInterval{From: left1, To: right3}
...
func TestTimeIntervalAddSameAndDifferentStartTimes(t *testing.T) {
	...
	ti2 := TimeInterval{From: left2, To: right1}
	ti3 := TimeInterval{From: left3, To: right2}
	ti4 := TimeInterval{From: left4, To: right3}

	ti5 := TimeInterval{From: left5, To: right1}
...
func TestTimeIntervalSearchSimple(t *testing.T) {
	...
	tis = tis.Add(left1, right1, nil)
	tis = tis.Add(left2, right2, nil)
	tis = tis.Add(left3, right3, nil)

	i := tis.search(left1, right1)
	if i != 0 {
		t.Fatalf("Search for item in entry 0 failed.")
	}

	i = tis.search(left2, right2)
	if i != 1 {
		t.Fatalf("Search for item in entry 1 failed.")
	}

	i = tis.search(left3, right3)
	if i != 2 {
...
func searchTestIntervals(description string, t *testing.T, intervals []TimeInterval, q time.Time, expectedMatches []TimeInterval) {
	...
	for i, ti := range matches {
		if ti.From != expectedMatches[i].From || ti.To != expectedMatches[i].To {
...
func TestTimeIntervalSearchComplex(t *testing.T) {
	...
	ti2 := TimeInterval{From: left2, To: right2}
	ti3 := TimeInterval{From: left3, To: right3}
	ti4 := TimeInterval{From: left4, To: right4}
	ti5 := TimeInterval{From: left5, To: right5}
	ti6 := TimeInterval{From: left6, To: right6}
	ti7 := TimeInterval{From: left7, To: right7}
	ti8 := TimeInterval{From: left8, To: right8}
	ti9 := TimeInterval{From: left9, To: right9}
